...
if [ "$actualformat" == "c" ]; then
    ...
    numat=$(head -1 "$cifile")
    sed -n '3,'"$((numat+2))"'p' "$cifile" | gawk '{print $1}' > "${tmpfille}.wytmp"
    cat "${tmpfille}.wytmp" | while read un; do
        grep -nm 1 "^${un}$" $HOME/.pertanu | cut -d : -f 1  | gawk '{printf "%-2s\n", $1}' >> "${tmpfille}.wyel"
    done
    sed -n '3,'"$((numat+2))"'p' "$cifile" | gawk '{printf "%8.5f  %8.5f  %8.5f\n", $2, $3, $4}' > "${tmpfille}.wypos"
fi
#echo "===DEBUG: WUEL"
#cat "${tmpfille}.wyel"
#echo "===DEBUG: WUPOS"
...
if [ "$actualformat" == "c" ]; then
    ...
    spgrp=1
    echo "pre2crys -g $spgrp -n $numel -w $wypos"
    rm "${tmpfille}.wytmp"
fi
